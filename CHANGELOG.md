...
### Added
- Configuration validation with automatic defaults for invalid values
- Thread-safe error logging in HNSW search
- Wing/room filtering support in HNSW search (post-search filtering)
- Config validation tests

### Changed
- Async processor marked as not fully implemented with clear warnings
- HNSW search now returns errors for diagnostics instead of silently failing
- Improved error handling throughout vector stores
...
- HNSW Vector Index: Fully integrated Hierarchical Navigable Small World graph
  ...
  - Background index building from SQLite on startup
...
### Changed
- **Enhanced `mira_status`**: Now includes performance metrics
  - Store/Recall operation counts
  - Latency percentiles (p50, p95)
  - Active searches count
- **Updated Configuration**: Added sections for metrics, webhooks, hnsw
- **CLI Version**: Updated to v0.3.0
...
[Unreleased]: https://github.com/benoitpetit/mira/compare/v0.3.0...HEAD
